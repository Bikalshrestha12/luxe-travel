...
export default function AIPlannerPage() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I'm having trouble connecting. Please ensure your API key is configured and try again.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <div
        className="max-w-350 mx-auto w-full px-6 lg:px-12 flex flex-col flex-1"
      ...
      >
        ...
        <motion.div
          ...
          style={{ borderColor: "rgba(200,146,15,0.1)" }}
        >
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              Powered by Claude · Trained on elite travel expertise
            </p>
          </div>
          ...
          >
            {QUICK_PROMPTS.map((p) => (
              <button
                ...
                className="px-4 py-2.5 rounded-full text-xs transition-all duration-300 text-left"
                style={{
        ...
        <div
          ...
          style={{
            scrollbarWidth: "thin",
            scrollbarColor: "rgba(200,146,15,0.3) transparent",
          }}
        >
          <AnimatePresence initial={false}>
            {messages.map((msg, i) => (
              ...
              >
                {/* Avatar */}
                <div
                  className={`w-9 h-9 rounded-full flex-none flex items-center justify-center text-xs font-medium ${msg.role === "user" ? "" : ""}`}
                  style={{
                    background:
                      msg.role === "assistant"
                        ? "linear-gradient(135deg, #7c3aed, #c8920f)"
                ...
                {/* Bubble */}
                <div
                  className={`max-w-2xl ${msg.role === "user" ? "items-end" : "items-start"} flex flex-col`}
                >
                  <div
                    className={`px-6 py-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap`}
                    style={{
                      background:
                        msg.role === "assistant"
                          ? "rgba(14,14,30,0.8)"
                          : "rgba(200,146,15,0.12)",
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  border: "1px solid rgba(200,146,15,0.1)",
                }}
              >
        ...
        >
          <div className="flex gap-3 items-end glass rounded-2xl px-5 py-4">
            ...
            <button
              ...
              className="w-10 h-10 rounded-full flex items-center justify-center flex-none transition-all duration-300 disabled:opacity-40"
              style={{
            ...
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path
                  d="M14 8L2 2l4 6-4 6 12-6z"
                  fill={input.trim() ? "#02020a" : "var(--gold)"}
                />
              </svg>
            </button>
          </div>
          ...
          >
            AI responses are for inspiration. Confirm all bookings with our
            specialists.
          </p>
        </div>
      </div>
